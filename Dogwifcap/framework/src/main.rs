...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let modules = vec!["dogwifcap"];
    let sources = vec!["dogwifcap"];
    ...
    let precompiled = sui_ctf_framework::get_precompiled(Path::new(&format!(
        "./chall/build/dogwifcap/sources/dependencies",
    )));
    ...
    for i in 0..modules.len() {
        ...
        let mod_path = format!("./chall/build/dogwifcap/bytecode_modules/{}.mv", module);
        let src_path = format!("./chall/build/dogwifcap/source_maps/{}.mvsm", module);
