...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let host = "37.27.26.101";
    let port = env::var("PORT").unwrap_or_else(|_| "31340".to_string());

    match TcpStream::connect(format!("{}:{}", host, port)) {
        Ok(mut stream) => {
            println!("  - Connected!");

            let mod_data : Vec<u8> = fs::read("./solve/build/zk_birthday_solution/bytecode_modules/zk_birthday_solution.mv").unwrap();
