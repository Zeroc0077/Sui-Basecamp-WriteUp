...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let modules = vec!["zk_birthday"];
    let sources = vec!["zk_birthday"];
    ...
    let precompiled = sui_ctf_framework::get_precompiled(Path::new(&format!(
        "./chall/build/zk_birthday/sources/dependencies",
    )));
    ...
    for i in 0..modules.len() {
        ...
        let mod_path = format!("./chall/build/zk_birthday/bytecode_modules/{}.mv", module);
        let src_path = format!("./chall/build/zk_birthday/source_maps/{}.mvsm", module);
    ...
    // Check Admin Account
    sui_ctf_framework::view_object(&mut adapter, FakeID::Enumerated(1, 0)).await;

    // Check Solution
    let mut args2: Vec<SuiValue> = Vec::new();
    let arg_ob2 = SuiValue::Object(FakeID::Enumerated(1, 0), None);
    args2.push(arg_ob2);
...
async fn main() -> Result<(), Box<dyn Error>> {
    // Create Socket - Port 31340
    let listener = TcpListener::bind("0.0.0.0:31340")?;
    println!("[SERVER] Starting server at port 31340!");
