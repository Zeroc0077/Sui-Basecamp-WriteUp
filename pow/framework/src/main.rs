...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let precompiled = sui_ctf_framework::get_precompiled(Path::new(&format!(
        "./chall/build/pow/sources/dependencies",
    )));
    ...
    for i in 0..modules.len() {
        ...
        let mod_path = format!("./chall/build/pow/bytecode_modules/{}.mv", module);
        let src_path = format!("./chall/build/pow/source_maps/{}.mvsm", module);
...
async fn main() -> Result<(), Box<dyn Error>> {
    // Create Socket - Port 31342
    let listener = TcpListener::bind("0.0.0.0:31342")?;
    println!("[SERVER] Starting server at port 31342!");
